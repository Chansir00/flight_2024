...
const float Control_Unit[18][20]=
{
  ...
  {1  ,1 ,0 ,0 ,0 ,0 , 0 ,500 ,0  ,0 , 500,   0.65   ,2.50  ,1.80  ,0  ,0 , 1000,  1 ,  1 ,  1 },//Pitch_Gyro;偏航角速度
  {1  ,1 ,0 ,0 ,0 ,0 , 0 ,30  ,0  ,0 , 100,   5.00   ,0.00  ,0.00  ,0  ,0 , 500,  1 ,  1 ,  1 },//Roll_Angle;横滚角
  {1  ,1 ,0 ,0 ,0 ,0 , 0 ,500 ,0  ,0 , 500,   0.65   ,2.50  ,1.80  ,0  ,0 , 1000,  1 ,  1 ,  1 },//Roll_Gyro;横滚角速度
  {1  ,1 ,0 ,0 ,0 ,0 , 0 ,45  ,0  ,0 , 150 ,  5.00   ,0.00  ,0.00  ,0  ,0 , 500,  1 ,  1 ,  1 },//Yaw_Angle;偏航角
  {1  ,1 ,0 ,0 ,0 ,0 , 0 ,300 ,0  ,0 , 500,   1.10   ,1.00  ,0.00  ,0  ,0 , 800,  1 ,  1 ,  1 },//Yaw_Gyro;偏航角速度 
  ...
  {1  ,1 ,0 ,0 ,0 ,0 , 0 ,200 ,0  ,0 ,50 ,   0.5     ,0.00   ,0    ,0   ,0 ,400,  1 ,  1 ,  1 },//High_Position;海拔高度位置
  {1  ,1 ,0 ,0 ,0 ,0 , 0 ,400 ,0  ,0 ,750 ,  1.5     ,6.00   ,1.8  ,0  ,0 ,1000,  1 ,  1 ,  1 },//High_Speed;海拔攀升速度
#else
  {1  ,1 ,0 ,0 ,0 ,0 , 0 ,200 ,0  ,0 ,100 ,  1.00     ,0.000   ,0    ,0  ,0 ,400,  1 ,  1 ,  1 },//High_Position;海拔高度位置
  {1  ,1 ,0 ,0 ,0 ,0 , 0 ,400 ,0  ,0 ,500 ,  5.00     ,0.000   ,0    ,0  ,0 ,800,  1 ,  1 ,  1 },//High_Speed;海拔攀升速度
#endif
  /*
  1偏差限幅标志；  2积分限幅标志；3积分分离标志；    4期望；
  5反馈            6偏差；        7上次偏差；       8偏差限幅值；
  ...
  */
  ...
  /*************加速度控制器****************/
  //期望最大加速度1000cm/s^2  10000
  {1  ,1 ,0 ,0 ,0 ,0 , 0 ,1000 ,0  ,0 ,750,0.10    ,2.5000    ,0   ,0   ,0 ,1000,  1 ,  1 ,  1 },//垂直加速度控制器	
		
  {1  ,1 ,0 ,0 ,0 ,0 , 0 ,100  ,0  ,0 ,3,  0.32    ,0.0000    ,0    ,0   ,0 ,150,   1 ,  1 ,  1 },//预留控制器1
  {1  ,1 ,0 ,0 ,0 ,0 , 0 ,100  ,0  ,0 ,15, 0.45    ,0.0000    ,0.0  ,0   ,0 ,25 ,   1 ,  1 ,  1 },//预留控制器2
		
		
  /*************光流位置、速度控制器****************/
  {1  ,1 ,0 ,0 ,0 ,0 , 0 ,100   ,15  ,0 ,15,  0.18  ,0.00   ,0    ,0   ,0 ,30,   1 ,  1 ,  1 },//光流位置
  {1  ,1 ,0 ,0 ,0 ,0 , 0 ,200   ,50  ,0 ,200, 4.00  ,0.03   ,0.0  ,0   ,0 ,450,  1 ,  1 ,  1 },//光流速度
		
  {1  ,1 ,0 ,0 ,0 ,0 , 0 ,50    ,30  ,0 ,100,  0.45  ,0.00   ,1.0  ,0    ,0 ,300  , 1 ,  1 ,  1 },//SDK位置
};
...
void Total_PID_Init(void)
{
  ...
  PID_Init(&Total_Controller.Reserve1_Control,Reserve1_Controler);
  PID_Init(&Total_Controller.Reserve2_Control,Reserve2_Controler);
  ...
  Set_Cutoff_Frequency(FC_Control_Freq, 50,&Control_Device_Err_LPF_Parameter);
  Set_Cutoff_Frequency(FC_Control_Freq, 20,&Control_Device_Div_LPF_Parameter);
  Set_Cutoff_Frequency(FC_Control_Freq, 15,&Control_Device_Div_LPF_Parameter_For_Gyro);//30
}
...
float PID_Control_SDK_Err_LPF(PID_Controler *Controler,uint8_t Differential_Enable_Flag)
{
  ...
  if(controller_dt<0.0025f) return 0;
  /*******偏差计算*********************/
  Controler->Last_Err=Controler->Err;//保存上次偏差
  Controler->Err=Controler->Expect-Controler->FeedBack;//期望减去反馈得到偏差
  ...
  Controler->Control_OutPut=Controler->Kp*Controler->Err_LPF//比例
													 ...
													 +Controler->Kd*Controler->Dis_Err_LPF;//已对偏差低通，此处不再对微分项单独低通
  /*******总输出限幅*********************/
  if(Controler->Control_OutPut>= Controler->Control_OutPut_Limit) Controler->Control_OutPut= Controler->Control_OutPut_Limit;
  if(Controler->Control_OutPut<=-Controler->Control_OutPut_Limit) Controler->Control_OutPut=-Controler->Control_OutPut_Limit;
  /*******返回总输出*********************/
  return Controler->Control_OutPut;
}
...
void PID_Paramter_Init_With_Flash()
{
  ...
  {
    if(ReadFlashParameterThree(PID1_PARAMETER_KP+3*i,
                            &PID_Parameter[i].p,
                            &PID_Parameter[i].i,
                            &PID_Parameter[i].d)==1)
