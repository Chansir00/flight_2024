...
void UpdateStatus()
{
    ...
    case ActionGoForward:
        ...
        break;
    case ActionGoLeft:
        ActionHoldPoint(MAX_HOVER_ERR, 500, ActionGoBack);
        break;
		case ActionGoBack:
			  ActionHoldPoint(MAX_HOVER_ERR, 500, ActionGoRight);
		    break;
		case ActionGoRight:
			  ActionHoldPoint(MAX_HOVER_ERR, 500, ActionGoRound);
			  break;
    case ActionGoRound:
        if(FollowManager.ActionComplete)
        {
            FollowManager.ActionList = ActionGoForward2;
        }
        break;
    case ActionGoForward2:
...
void UpdateAction(float dt)
{
    ...
    case ActionGoForward:
        ...
        break;
    case ActionGoForward2:
        if (FollowManager.ptrFrame->FormType == Vertical)
        {
            PIDGroup[emPID_FolloLinePosVertically].desired = 120 / 2;
            PIDGroup[emPID_FolloLinePosHorizontally].desired = 160 / 2;
        ...
        if(FollowManager.ActionList == ActionGoForward || FollowManager.ActionList == ActionGoForward2)
        {
    ...
    case ActionGoLeft:
        ...
        break;
    case ActionGoRound:
        FollowManager.ActionComplete = ActionGoZ(180);
        break;
    case ActionGoRight:
        program_ctrl.vel_cmps_h[X] = 0;
        program_ctrl.vel_cmps_h[Y] = -20;
    ...
    if(FollowManager.ptrFrame->FormType == 101)
    {
         P2OUT |= GPIO_PIN1;
    }else
    {
